...
def draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels,height,frameCount,labelh):
    ...
    if len(idxs) > 0:
        for i in idxs.flatten():
            ...
            print("yolo output :",labels[classids[i]])
            if 0 in classids: #Check the detected item is smoking
                if(labelh == 'smoking' or labelh == 'smoking hookah'):
                    add_label(img,height,'smoke.png')
                    labelledImg = cv.imread("pasted_image.jpg")
                    detect = 1
            elif 1 in classids:
                if(labelh=='bike riding' or labelh == 'scooter riding'):   
                    labelledImg = cv.imread("pasted_image.jpg")
                    detect =2
            else:
                labelledImg = img
...
def infer_image(net, layer_names, height, width, img, colors, labels, FLAGS,frameCount,labelh,boxes=None, confidences=None, classids=None, idxs=None, infer=True):
    ...
    img,detect = draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels,height,frameCount,labelh)
